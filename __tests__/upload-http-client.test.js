...
test('upload test', async () => {
  ...
  const fakeSignedUploadUrl =
    'https://fakestorageaccount.blob.core.windows.net/container/blob?sv=2020-08-04&st=2023-04-13T18%3A00%3A00Z&se=2023-04-13T19%3A00%3A00Z&sr=b&sp=rw&sig=fakeSignature'

  const mockserver = nock('http://localhost:12345').persist()
  mockserver.post('/twirp/github.actions.results.api.v1.ArtifactService/CreateArtifact').reply(200, {
    ok: true,
    signedUploadUrl: fakeSignedUploadUrl
  })

  mockserver.post('/twirp/github.actions.results.api.v1.ArtifactService/FinalizeArtifact').query(true).reply(200, {
    ok: true,
    artifactId: '123'
  })
